...
- `campaigns` / `campaign_versions` / `series` (graph JSONB) / `sends` (**hard `UNIQUE (workspace_id, campaign_id, contact_id)`** — re-fires safe; the exactly-once claim slot) / `message_events` (delivery lifecycle, monthly partitions, rolled into `campaign_stats`) / `subscription_types` + `consents` (per contact × type, audit-trailed via append-only `consent_events` + a current-state projection). **P1.8 note:** `sends` is deliberately **non-partitioned** — a partitioned UNIQUE must include the partition key, which would let a later-month re-fire duplicate; correctness (the cross-partition unique) wins over the scale optimisation, and `sends` is a per-(campaign,contact) ledger (not an event firehose), mitigated with `CONCURRENTLY` indexes (it stays in `check_migrations.LARGE_TABLES`). `message_events` is the partitioned firehose (email + in-app posts/chats: `source_kind ∈ email|post|chat`). Send-rate uses per-tenant + global **Lua token buckets**; opens/clicks are counted **unique-per-contact**; marketing consent is **opt-out** by default (a `subscription_types.requires_opt_in` flag switches to GDPR opt-in). In-app posts add `posts` + `post_receipts` (the in-app exactly-once ledger); an unsubscribe uses stateless HMAC List-Unsubscribe tokens (RFC 8058 one-click).
