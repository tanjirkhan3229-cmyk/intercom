...
Rationale: Dev A's prompts chain through the Aide/retrieval/channel stack (deep context compounds); Dev B's chain through the engine/product surfaces (workflow engine → Series → surfaces reuse each other). Cross-track dependencies reduce to three explicit interface contracts (§4).
...
| 2 | **P1.2** Aide orchestrator v1 | **P1.6** Workflow builder UI |
| 3 | **P1.3** Aide product surface + metering | **P1.7** Inbox v2 (SLAs, views, balanced, collision) |
| 4 | **P1.4** Aide analytics v0 | **P1.8** Outbound v1 (consents, posts, broadcasts) |
| 5 | **P1.10** Mobile SDKs beta (slack-time item; drop to phase 2 if Aide slips) | **P1.9** Segments, imports, Slack/Zapier |
| 6 | **P1.11 Gate — together** (Aide measurement is A, chaos/load is B) | ← same |

Notes: P1.5's "hand to Aide" action ships behind a flag until sync point S1. P1.7's SLA timers reuse P1.5's `timers` infra — B sequences internally, no cross-block. P1.3 needs nothing from B (billing meters exist from P0.10).
...
| 2 | **P2.2** SMS + Aide on email/WhatsApp | **P2.4** Series builder |
| 3 | **P2.7** Aide Actions + custom answers | **P2.5** Outbound surfaces (surveys, tours, banners, checklists, push) |
...
| 2 | **P3.2** Aide v3: procedures, guidance, evals, content gaps | **P3.4** Warehouse export + multibrand |
...
|---|---|---|---|
| C1 | `hand_to_aide(conversation_id, context)` service interface + Aide handoff event (`ai_status` transitions, summary-note shape) | A → B (workflow action, routing rules) | S1 |
| C2 | Predicate AST + audience components (segments/views/targeting share one engine) | B → A (knowledge audience scoping, Aide eligibility rules) | End of P1.5 |
...
- **S1 (≈ week 3 of phase 1):** P1.2 + P1.5 both landed → wire C1, unflag hand-to-Aide, e2e test together.
...
- **Git:** trunk-based; branches `feat/p<prompt>-<slug>` (e.g. `feat/p1.2-aide-orchestrator`); PRs ≤ ~800 lines reviewed by the other dev within 24 h; rebase, don't merge-commit; **never run two agents' git operations in the working tree simultaneously** (use separate clones/worktrees per dev).
